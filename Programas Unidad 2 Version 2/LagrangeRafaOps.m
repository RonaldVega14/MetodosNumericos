%Método de Lagrange - Tipo guión
clear all %#ok<CLALL> (Matlab lo puso)
clc
disp('Método de Lagrange con opciones');
disp(' ');
z=input('Introduzca el valor de x a aproximar: ');
n=input('¿Cuántos valores de X desea ingresar?: ');
for i=0:(n-1)
   fprintf ('  x%0.0f : ',i);
   vecx(i+1)=input (''); %#ok<*SAGROW> (Matlab lo puso)
end
fprintf('¿Qué desea hacer:\n1.Agregar los valores de F(x).\n2.Introducir una función F(x).\nR/')
op=input('');
...
fprintf('Error: %e\n',err);
    ...
    fprintf('Debe ingresar una opción válida (1 ó 2)');
end